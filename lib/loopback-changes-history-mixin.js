...
module.exports = function ChangesHistoryMixin (Model, bootOptions) {
  ...
  Model.observe('before save', function (ctx) {
    if (ctx.options.debug){
      console.log('before', Object.keys(ctx))
      console.log('before ctx.isNewInstance', ctx.isNewInstance)
      console.log('before ctx.instance', ctx.instance)
      console.log('before ctx.currentInstance', ctx.currentInstance)
      console.log('before ctx.data', ctx.data)
      console.log('before ctx.where', ctx.where)
    }
    return Promise.resolve()
    .then(() => {
      const instance = ctx.instance || ctx.currentInstance;
      if (instance) return instance;
      ctx.hookState.unknowInstance = true;
