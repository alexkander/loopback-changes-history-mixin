...
module.exports = function ChangesHistoryMixin(Model, bootOptions) {
  ...
  options.modelName = `${Model.modelName}_history`;
  Object.assign(options, bootOptions);

  function assert(cond, code, str) {
    if (!cond) {
      const err = new Error(
        `${Model.modelName} loopback-allowed-properties-mixin: ${str}`
      );
      err.code = code;
      throw err;
    }
  ...
  if (options.fields === '*') {
    options.fields = [];
    Model.forEachProperty((fieldName, _opts) => {
  ...
  assert(
    ...
    'fieldMustHaveAtLeastAElement',
    'fields must have at least a element'
  );
  assert(
    typeof options.modelName === 'string',
    'modelNameMustBeAString',
    'modelName must be a string'
  );
  assert(
    typeof options.relationName === 'string',
    'relationNameMustBeAString',
    'relationName must be a string'
  );
  assert(
    typeof options.relationForeignKey === 'string',
    'relationForeignKeyMustBeAString',
    'relationForeignKey must be a string'
  );
  assert(
    typeof options.relationParentName === 'string',
    'relationParentNameMustBeAString',
    'relationParentName must be a string'
  );
  assert(
    typeof options.versionFieldName === 'string',
    'versionFieldNameMustBeAString',
    'versionFieldName must be a string'
  );
  assert(
    typeof options.versionFieldLen === 'number',
    'versionFieldLenMustBeANumber',
    'versionFieldLen must be a number'
  );
  assert(
    options.hashFieldName === false ||
      typeof options.hashFieldName === 'string',
    'hashFieldNameMustBeAString',
    'hashFieldName must be a string'
  );
  if (options.hashFieldName !== false) {
    assert(
      typeof options.hashFieldLen === 'number',
      'hashFieldLenMustBeANumber',
      'hashFieldLen must be a number'
    );
  }
  assert(
    options.actionFieldName === false ||
      typeof options.actionFieldName === 'string',
    'actionFieldNameMustBeAString',
    'actionFieldName must be a string'
  );
  assert(
    options.updatedFieldName === false ||
      typeof options.updatedFieldName === 'string',
    'updatedFieldNameMustBeAString',
    'updatedFieldName must be a string'
  );
  ...
  options.fields = options.fields.filter(
    (fieldName) => fieldName !== idFieldName
  );
  ...
  function getVersioningData(instance, data = {}) {
    ...
    const dataRaw = instance || {};
    options.fields.map((fieldName) => {
      versionedData[fieldName] =
        fieldName in data ? data[fieldName] : dataRaw[fieldName];
    });
  ...
  historyFields[options.versionFieldName] = {
    type: 'string',
    length: options.versionFieldLen,
  };

  if (options.hashFieldName) {
    historyFields[options.hashFieldName] = {
      type: 'string',
      length: options.hashFieldLen,
    };
    Model.defineProperty(options.hashFieldName, {
      type: 'string',
      length: options.hashFieldLen,
    });
  }
  ...
  const HistoryModel = Model.dataSource.createModel(
    options.modelName,
    historyFields
  );
    ...
    return Promise.resolve()
      ...
      .then((instance) => {
        ...
        if (ctx.hookState.needNewVersion) {
          data[options.versionFieldName] = getNewVersion(
            instance,
            ctx.isNewInstance
          );
        }
  ...
  Model.observe('after save', function (ctx) {
    return Promise.resolve().then(() => {
      if (ctx.hookState.needNewVersion && !ctx.where) {
        ...
        versionedData[options.versionFieldName] =
          instance[options.versionFieldName];
        if (options.actionFieldName) {
          versionedData[options.actionFieldName] = ctx.isNewInstance
            ? 'create'
            : 'update';
        }
        if (options.hashFieldName) {
          versionedData[options.hashFieldName] =
            instance[options.hashFieldName];
        }
        if (options.updatedFieldName) {
          versionedData[options.updatedFieldName] = new Date();
        }
        return ctx.instance[options.relationName]
  ...
  Model.observe('after delete', function (ctx) {
    return Promise.resolve().then(() => {
      ...
      return HistoryModel.create(ctx.hookState.values).then((version) => {
        Model.emit('version.record.created', {
          version,
          instance: ctx.hookState.instance,
        });
      });
    });
  });
